...
//
// Primary origin is APP_URL (or DODO_APP_URL fallback). Optional additional
// origins (e.g. http://localhost:5173 for admin dev-testing against live
// functions) come from APP_URL_EXTRA_ORIGINS — comma-separated, no trailing
// slash. Anything not on the allowlist gets `Access-Control-Allow-Origin:
// null`, which fails the browser's CORS check rather than silently allowing.
...
const _primary = _appUrl ?? 'http://localhost:5173';
const _extras = (Deno.env.get('APP_URL_EXTRA_ORIGINS') ?? '')
  .split(',')
  .map((s) => s.trim())
  .filter(Boolean);
const ALLOWED_ORIGINS = new Set<string>([_primary, ..._extras]);

// Per-request CORS headers. Echoes the request's origin only when it's in
// ALLOWED_ORIGINS — any other origin gets `Access-Control-Allow-Origin: null`,
